...
import "src/errors/ZKErrors.sol";
...
// A variation of Op-succinct: https://github.com/succinctlabs/op-succinct/blob/main/contracts/src/fp/OPSuccinctFaultDisputeGame.sol
//
// The idea is to make the bond requirement on the honest party constant instead of linear.
// Instead of having to challenge every invalid proposal, we just ensure that they cannot update 
// the anchor.
//
// Changes:
// 1. The block number of the proposal between a ZK game and its parent is fixed
// 2. Within the `resolve()` function, if the ZK game's proposal's block number is less than or 
//    equal to the current anchor's block number in the anchor state registry, the game's status 
//    cannot update and all bonds are refunded
//
// A proposal cannot update the anchor if
// - A dispute game cannot resolve/resolves to CHALLENGER_WINS or
// - If the proposal's block number is less than or equal to the current anchor's block number
//
// The above conditions mean that if several invalid games are proposed for the next block interval, 
// proving the correct proposal to update the anchor is sufficient as attackers can only rely on 
// an optimistic resolution, which takes much longer.
// 
// Then, an attacker can only make a long chain of invalid games to try and update the state. Again, 
// by making one correct proposal and proving it, the first game in this invalid chain cannot resolve 
// to DEFENDER_WINS so the rest of the games in the chain cannot resolve. 
//
// We can also challenge some invalid games to acquire bonds instead of challenging everything.

contract ZKDisputeGame is Clone {
    ////////////////////////////////////////////////////////////////
    //                         Enums                              //
    ////////////////////////////////////////////////////////////////
    ...
    event Nullified(Claim indexed rootClaim, Claim indexed nullifiedRootClaim);
    ...
    IZKVerifier internal immutable VERIFIER;
    ...
    constructor(
        ...
        IRelayAnchorStateRegistry _anchorStateRegistry,
        uint256 _l2ChainId,
    ...
    function initialize() external payable virtual {
        ...
        if (parentIndex() != type(uint32).max) {
            ...
            startingOutputRoot = Proposal({
                l2SequenceNumber: ZKDisputeGame(address(proxy)).l2SequenceNumber(),
                root: Hash.wrap(ZKDisputeGame(address(proxy)).rootClaim().raw())
            });
            ...
            (startingOutputRoot.root, startingOutputRoot.l2SequenceNumber) =
                IRelayAnchorStateRegistry(ANCHOR_STATE_REGISTRY).getAnchorRoot();
        }

        // INVARIANT: The game must be initialized with a root claim that is at least one block after the starting output root.
        ...
        finalizationDelay = ANCHOR_STATE_REGISTRY.finalityDelay(GAME_TYPE);
    }
    ...
    function prove(bytes calldata proofBytes) external returns (ProposalStatus) {
        ...
        VERIFIER.verify(proofBytes, rootClaim(), l2SequenceNumber());
    ...
    function resolve() external returns (GameStatus) {
        ...
        if (parentGameStatus == GameStatus.CHALLENGER_WINS) {
            ...
            if (claimData.status == ProposalStatus.Unchallenged) {
                // If the game's proposal's block number is less than or equal to the current anchor's block number,
                // the game's status cannot update and all bonds are refunded.
                ( , uint256 currentAnchorBlockNumber) = ANCHOR_STATE_REGISTRY.getAnchorRoot();
                if (l2SequenceNumber() <= currentAnchorBlockNumber) {
    ...
    function nullify(bytes calldata _proof, Claim _rootClaim) external {
        require(_rootClaim.raw() != rootClaim().raw(), "ZK: Invalid root claim");
        ...
        ANCHOR_STATE_REGISTRY.nullify();

        emit Nullified(rootClaim(), _rootClaim);
    }
    ...
    function verifier() external view returns (IZKVerifier verifier_) {
        verifier_ = VERIFIER;
    }
}